...
char **myargv;
bool dorehash = NO;
bool dorehashbans = NO;
bool doremotd = NO;
bool kline_queued = NO;
...
ircd_shutdown(const char *reason)
{
	...
	RB_DLINK_FOREACH(ptr, lclient_list.head)
	{
		...
		sendto_one(target_p, ":%s NOTICE %s :Server Terminating. %s", me.name,
...
print_startup(int pid)
{
	inotice("now running in %s mode from %s as pid %d ...",
		!server_state_foreground ? "background" : "foreground", ConfigFileEntry.dpath, pid);
...
init_sys(void)
{
	...
	if(!getrlimit(RLIMIT_NOFILE, &limit))
	{
		...
		if(maxconnections <= MAX_BUFFER)
		{
			...
			fprintf(stderr,
				"ERROR: SporksIRCD reserves %d FDs, shell limits must be above this\n",
				MAX_BUFFER);
			exit(EXIT_FAILURE);
		}
		return;
...
initialize_global_set_options(void)
{
	...
	if(ConfigFileEntry.default_operhost)
		rb_strlcpy(GlobalSetOptions.operhost, ConfigFileEntry.default_operhost,
	...
	rb_strlcpy(GlobalSetOptions.adminstring, ConfigFileEntry.default_adminstring,
...
{
	/* These are core capabs */
	add_capability("QS", CAP_QS, YES, YES);
	add_capability("EX", CAP_EX, YES, NO);
	add_capability("IE", CAP_IE, YES, NO);
	add_capability("CHW", CAP_CHW, YES, NO);
	add_capability("EOPMOD", CAP_EOPMOD, YES, NO);
	add_capability("ZIP", CAP_ZIP, NO, NO);
	add_capability("CLUSTER", CAP_CLUSTER, YES, NO);
	add_capability("BAN", CAP_BAN, YES, NO);
}
...
write_pidfile(const char *filename)
{
	...
	if((fb = fopen(filename, "w")))
	{
		...
		if((fputs(buff, fb) == -1))
		{
			ilog(L_MAIN, "Error writing %u to pid file %s (%s)", pid, filename,
			     strerror(errno));
		}
		fclose(fb);
		return;
 ...
 * error inside libratbox, and we don't know how to handle the
...
ircd_die_cb(const char *str)
{
	if(str != NULL)
	{
		...
		sendto_realops_snomask(SNO_GENERAL, L_NETWIDE,
				       "libratbox has called the die callback..aborting: %s", str);
		ierror("libratbox has called the die callback..aborting: %s", str);
	}
	else
		ierror("libratbox has called the die callback..aborting");
...
ircd_main(int argc, char *argv[])
{
	...
	rb_lib_init(ircd_log_cb, ircd_restart_cb, ircd_die_cb, !server_state_foreground,
		    maxconnections, DNODE_HEAP_SIZE, FD_HEAP_SIZE);
	...
	privilegeset_set_new("default", NULL, "", 0);
	...
	mod_add_path(MODULE_DIR "/extensions");
#endif
	...
	/* Cache ISUPPORT and turn on autoregen */
	cache_isupport();
	isupport_regen = YES;
