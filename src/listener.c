...
static int accept_precallback(rb_fde_t * F, struct sockaddr *addr, rb_socklen_t addrlen,
			      void *data);
static void accept_callback(rb_fde_t * F, int status, struct sockaddr *addr, rb_socklen_t addrlen,
			    void *data);
	...
	for(listener = ListenerPollList; listener; listener = listener->next)
	{
		sendto_one_numeric(source_p, RPL_STATSPLINE,
				   ...
				   ntohs(listener->addr.ss_family ==
					 AF_INET ? ((struct sockaddr_in *) &listener->
						    addr)->sin_port : ((struct sockaddr_in6 *)
...
inetport(struct Listener *listener)
{
	...
	if(listener->addr.ss_family == AF_INET6)
	{
		...
		{
			rb_inet_ntop(AF_INET6, &in6->sin6_addr, listener->vhost,
	...
	else
	{
		...
		{
			rb_inet_ntop(AF_INET, &in->sin_addr, listener->vhost,
				     sizeof(listener->vhost));
			listener->name = listener->vhost;
		}
	}
...
add_listener(int port, const char *vhost_ip, int family, int ssl)
{
	...
	if(vhost_ip != NULL)
	{
		...
		{
			if(rb_inet_pton
			   (family, vhost_ip, &((struct sockaddr_in *) &vaddr)->sin_addr) <= 0)
				return;
		}
		else
		{
			if(rb_inet_pton
			   (family, vhost_ip, &((struct sockaddr_in6 *) &vaddr)->sin6_addr) <= 0)
				return;
...
accept_precallback(rb_fde_t * F, struct sockaddr *addr, rb_socklen_t addrlen, void *data)
{
	...
	if((maxconnections - 10) < rb_get_fd(F))	/* XXX this is kinda bogus */
	{
		...
		{
			sendto_realops_snomask(SNO_GENERAL, L_ALL, "All connections in use. (%s)",
