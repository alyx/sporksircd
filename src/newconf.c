...
static bool yy_blacklist_ipv4 = 1;
static bool yy_blacklist_ipv6 = 0;

static char *yy_privset_extends = NULL;
static char *yy_privset_operstring = NULL;
...
{
	struct TopConf *tc;
	rb_dlink_node *tptr, *iptr, *nptr;

	if((tc = find_top_conf(name)) == NULL)
		return -1;

	if((tptr = rb_dlinkFind(tc, &conf_items)) == NULL)
		return -1;

	rb_dlinkDestroy(tptr, &conf_items);

	/* Destroy all sub-items that may still exist --Elizabeth */
	RB_DLINK_FOREACH_SAFE(iptr, nptr, tc->tc_items.head)
	{
		rb_dlinkDestroy(iptr, &tc->tc_items);
	}
...
conf_set_serverinfo_sid(void *data)
{
	...
	if(ServerInfo.sid[0] == '\0')
	{
		...
		{
			conf_report_error("Ignoring serverinfo::sid " "-- bogus sid.");
			return;
		}
...
static void
conf_set_privset_operstring(void *data)
{
	if(yy_privset_operstring)
		rb_free(yy_privset_operstring);

	if(strlen((char *) data) < 400)
		yy_privset_operstring = rb_strdup((char *) data);
	else
		conf_report_error("Ignoring operstring setting for privset %s -- operstring too long.",
			 conf_cur_block_name);
}
...
conf_set_privset_privs(void *data)
{
	...
	if(privs)
	{
		if(yy_privset_extends)
		{
			...
			if(!set)
			{
				conf_report_error("Warning -- unknown parent privilege set %s for %s; assuming defaults",
					 yy_privset_extends, conf_cur_block_name);

				set = privilegeset_get("default");
			}

			privilegeset_extend(set,
					    conf_cur_block_name !=
					    NULL ? conf_cur_block_name : "<unknown>",
					    yy_privset_operstring, privs, 0);
		...
		{
			privilegeset_set_new(conf_cur_block_name !=
					     NULL ? conf_cur_block_name : "<unknown>",
					     yy_privset_operstring, privs, 0);
		}

		rb_free(yy_privset_operstring);
		yy_privset_operstring = NULL;
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, yy_oper_list.head)
	{
		...
		if(!EmptyString(yy_oper->vhost))
		{
			if(valid_hostname(yy_oper->vhost))
				...
				conf_report_error("Ignoring vhost setting for oper %s -- invalid hostmask.",
					 yy_oper->name);
		}
		...
		if(!EmptyString(yy_oper->swhois))
		{
			if(strlen(yy_oper->swhois) < 400)
				...
				conf_report_error("Ignoring swhois setting for oper %s -- swhois too long.",
					 yy_oper->name);
		}
		...
		if(!EmptyString(yy_oper->operstring))
		{
			if(strlen(yy_oper->operstring) < 400)
				...
				conf_report_error("Ignoring operstring setting for oper %s -- operstring too long.",
					 yy_oper->name);
		}
		...
		if(!EmptyString(yy_oper->account))
			yy_tmpoper->account = rb_strdup(yy_oper->account);
#endif
...
static void
conf_set_oper_account(void *data)
{
	if(yy_oper->account)
		rb_free(yy_oper->account);

	yy_oper->account = rb_strdup((char *) data);
};
...
conf_set_class_cidr_ipv4_bitlen(void *data)
{
	...
	if(*(unsigned int *) data > maxsize)
		conf_report_error("class::cidr_ipv4_bitlen argument exceeds maxsize (%d > %d) - ignoring.",
...
conf_set_class_cidr_ipv6_bitlen(void *data)
{
	...
	if(*(unsigned int *) data > maxsize)
		conf_report_error("class::cidr_ipv6_bitlen argument exceeds maxsize (%d > %d) - ignoring.",
			 *(unsigned int *) data, maxsize);
	else
...
conf_end_auth(struct TopConf *tc)
{
	...
	if((found_conf = find_exact_conf_by_address("*", CONF_CLIENT, "*")) &&
	   found_conf->spasswd == NULL)
	{
		...
		(!found_conf->spasswd || (yy_aconf->spasswd && 0 == irccmp(found_conf->spasswd, yy_aconf->spasswd))))
	{
		conf_report_error("Ignoring duplicate auth block for %s@%s", yy_aconf->user,
				  yy_aconf->host);
	}
	else
		add_conf_by_address(yy_aconf->host, CONF_CLIENT, yy_aconf->user, yy_aconf->spasswd,
				    yy_aconf);
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, yy_aconf_list.head)
	{
		...
		if(find_exact_conf_by_address("*", CONF_CLIENT, "*"))
			...
			conf_report_error("Ignoring duplicate auth block for %s@%s", yy_tmp->user,
					  yy_tmp->host);
		else
			add_conf_by_address(yy_tmp->host, CONF_CLIENT, yy_tmp->user,
					    yy_tmp->spasswd, yy_tmp);
		rb_dlinkDestroy(ptr, &yy_aconf_list);
	}
...
conf_end_connect(struct TopConf *tc)
{
	...
	{
		conf_report_error
			("Ignoring connect block for %s -- no fingerprint or password credentials provided.",
			 yy_server->name);
		return 0;
	}
...
conf_set_connect_vhost(void *data)
{
	...
	{
		conf_report_error("Invalid netmask for server vhost (%s)", (char *) data);
		return;
	}
...
conf_set_service_name(void *data)
{
	...
	for(s = data; *s != '\0'; s++)
	{
		if(!IsServChar(*s))
		{
			conf_report_error("Ignoring service::name " "-- bogus servername.");
			return;
		}
		else if(*s == '.')
			dots++;
...
conf_set_blacklist_type(void *data)
{
	...
	/* Don't assume we have either */
	yy_blacklist_ipv4 = NO;
	yy_blacklist_ipv6 = NO;

	for(; args; args = args->next)
	{
		if(!strcasecmp(args->v.string, "ipv4"))
			yy_blacklist_ipv4 = YES;
		else if(!strcasecmp(args->v.string, "ipv6"))
			yy_blacklist_ipv6 = YES;
		else
	...
	{
		conf_report_error
			("blacklist::type has neither IPv4 nor IPv6! Defaulting to IPv4...");
		yy_blacklist_ipv4 = YES;
	}
}
...
conf_set_blacklist_reason(void *data)
{
	...
	if(yy_blacklist_host && yy_blacklist_reason)
	{
		...
		new_blacklist(yy_blacklist_host, yy_blacklist_reason, yy_blacklist_ipv4,
			      yy_blacklist_ipv6);
...
conf_report_error(const char *fmt, ...)
{
	...
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "\"%s\", line %d: %s", current_file, lineno + 1,
			       msg);
}
...
static void
conf_set_generic_bool(void *data, void *location)
{
	*((bool *) location) = *((bool *) data);
}
...
conf_call_set(struct TopConf *tc, char *item, conf_parm_t * value, int type)
{
	...
	{
		conf_report_error("Non-existant configuration setting %s::%s.", tc->tc_name,
				  (char *) item);
		return -1;
	}
	...
	{
		conf_report_error("Option %s::%s does not take a list of values.", tc->tc_name,
				  item);
		return -1;
	}
	...
	if(CF_TYPE(value->v.list->type) != CF_TYPE(cf->cf_type))
	{
		 ...
		 * convert it back
		 */
		if((CF_TYPE(value->v.list->type) == CF_YESNO) && (CF_TYPE(cf->cf_type) == CF_STRING))
		{
		...
		else if(!((CF_TYPE(value->v.list->type) == CF_INT) && (CF_TYPE(cf->cf_type) == CF_TIME)))
		{
	...
	else
	{
		...
		case CF_YESNO:
			if(cf->cf_arg)
				conf_set_generic_bool(&cp->v.number, cf->cf_arg);
			else
		...
		case CF_QSTRING:
			if(EmptyString(cp->v.string))
				conf_report_error("Ignoring %s::%s -- empty field", tc->tc_name,
						  item);
			else if(cf->cf_arg)
...
static struct ConfEntry conf_operator_table[] =
{
	...
	{ "account",	CF_QSTRING, conf_set_oper_account,	0, NULL },
...
{
	{ "operstring",	CF_QSTRING,		conf_set_privset_operstring,	0, NULL },
...
static struct ConfEntry conf_channel_table[] =
{
	...
	{ "founder_on_channel_create", CF_YESNO, NULL, 0, &ConfigChannel.founder_on_channel_create },
	...
	{ "use_founder",		CF_YESNO, NULL, 0, &ConfigChannel.use_founder	},
