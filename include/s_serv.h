...
struct Capability
{
	rb_dlink_node node;	/* Our node */
	char *name;		/* name of capability */
	unsigned int cap;	/* mask value */
	bool required;  /* 1 if required, 0 if not */
};
...
#define CAP_REMOVE	0x00020 /* Can do REMOVE - XXX modularise me */
...
#define CAP_MS		0x800000 /* supports MODESUPPORT message */
...
extern void capab_update(void);
extern const char *show_capabilities(struct Client *client);
extern void try_connections(void *unused);

extern void add_capability(const char *capability, int flag, bool add_default, bool required);
extern void delete_capability(const char *capability);

extern int match_all_servers(const char *pattern);
...
extern void burst_modes_TS6(struct Client *client_p, struct Channel *chptr,
