...
DECLARE_MODULE_AV1(stats, NULL, NULL, stats_clist, stats_hlist, NULL, "SporksIRCD development team");
...
stats_operedup (struct Client *source_p)
{
	...
	RB_DLINK_FOREACH (oper_ptr, oper_list.head)
	{
		...
		/* Hide oper:hidden ops for non-ops */
		if(!IsOper(source_p) && IsOperHidden(target_p))
			continue;
	...
	sendto_one_numeric(source_p, RPL_STATSDEBUG,
				"p :%u idle/away staff members not shown",
				count);
...
stats_memory (struct Client *source_p)
{
	...
	int channel_list_modes = 0;

	int channel_list_items = 0;
	...
	RB_DLINK_FOREACH(ptr, global_channel_list.head)
	{
		...
		{
			actualModeItem = rb_dlink->data;
			channel_bans++;

			channel_ban_memory += sizeof(rb_dlink_node) + sizeof(struct mode_list_t);
		}
		...
		RB_DLINK_FOREACH(rb_dlink, chptr->exceptlist.head)
		{
			...
			channel_except_memory += (sizeof(rb_dlink_node) + sizeof(struct mode_list_t));
		}
		...
		RB_DLINK_FOREACH(rb_dlink, chptr->invexlist.head)
		{
			...
			channel_invex_memory += (sizeof(rb_dlink_node) + sizeof(struct mode_list_t));
		}
		...
		RB_DLINK_FOREACH(rb_dlink, chptr->quietlist.head)
		{
			...
			channel_quiet_memory += (sizeof(rb_dlink_node) + sizeof(struct mode_list_t));
		}

		/* Count up the other list classes
		 * XXX should give more detail */
		for(i = 0; i < 128; i++)
		{
			...
			rb_dlink_list *list = get_channel_list(chptr, mode->c);

			RB_DLINK_FOREACH(rb_dlink, list->head)
			{
				actualModeItem = rb_dlink->data;
				channel_list_items++;

				channel_list_memory += (sizeof(rb_dlink_node) + sizeof(struct mode_list_t));
			}
		}
	}
	...
	sendto_one_numeric(source_p, RPL_STATSDEBUG,
			   "z :Other list items (%d other list type(s)) %u(%d)",
			   channel_list_modes, channel_list_items,
			   (int) channel_list_memory);
