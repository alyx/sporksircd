...
DECLARE_MODULE_AV1(okick, NULL, NULL, okick_clist, NULL, NULL, "SporksIRCD development team");
...
mo_okick(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	*buf = '\0';
	...
	if(!(target_p = find_chasing(source_p, user, &chasing)))
	{
	...
	sendto_channel_local(ALL_MEMBERS, chptr, ":%s KICK %s %s :%s",
			     me.name, chptr->chname, target_p->name, comment);
	sendto_server(&me, chptr, CAP_TS6, NOCAPS,
		      ":%s KICK %s %s :%s", me.id, chptr->chname, target_p->id, comment);
	remove_user_from_channel(msptr);

	rb_snprintf(text, sizeof(text), "K%s", target_p->id);
