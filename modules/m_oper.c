...
DECLARE_MODULE_AV1(oper, NULL, NULL, oper_clist, NULL, NULL, "SporksIRCD development team");
...
m_oper(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if (oper_p->account != NULL)
	{
		if (irccmp(source_p->user->suser, oper_p->account) != 0)
		{
			sendto_one_numeric(source_p, ERR_NOOPERHOST, form_str(ERR_NOOPERHOST));
			ilog(L_FOPER, "FAILED OPER (%s) by (%s!%s@%s) (%s) -- account name mismatch",
			    name, source_p->name,
			    source_p->username, source_p->host, source_p->sockhost);
			...
			{
				sendto_realops_snomask(SNO_GENERAL, L_ALL,
						       "Failed OPER attempt - account name mismatch by %s (%s@%s)",
						       source_p->name, source_p->username, source_p->host);
			}
			return 0;
		}
	...
	if(match_oper_password(password, oper_p))
	{
		...
		/* Warn about possible use of DES */
		if (IsOperConfEncrypted(oper_p) && oper_p->passwd[0] != '$' &&
		    (oper_p->passwd[2] != '$' || oper_p->passwd[3] != '$'))
		{
			sendto_realops_snomask(SNO_GENERAL, L_NETWIDE,
					"Warning: Oper block %s may be using a DES encrypted password, "
					"which is considered insecure. Consider switching to SHA256.",
					name);
			sendto_one_notice(source_p, ":Your oper password may be insecure due to use of DES, an insecure "
						    "hashing algorithim which can be cracked. Consider switching to SHA256.");
		}
