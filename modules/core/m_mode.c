...
DECLARE_MODULE_AV1(mode, modinit, modfini, mode_clist, NULL, NULL, "SporksIRCD development team");

static int
modinit(void)
{
	add_capability("MLOCK", CAP_MLOCK, YES, NO);
	return 0;
}
...
ms_bmask(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	case 'q':
		...
		break;
	default:
		/* XXX should we blindly propagate this? */
		if (!(mode = get_list_mode(parv[3][0])))
			return 0;
		else
		{
			banlist = get_channel_list(chptr, parv[3][0]);
			mode_type = 0;
			mems = mode->mems;
	...
	while(!EmptyString(s))
	{
		...
		if(add_id(fakesource_p, chptr, s, forward, banlist, mode_type, item_callback))
		{
