 ...
 */
struct ConfItem *
find_conf_by_address(const char *name, const char *sockhost, const char *orighost,
				...
				for(arec = atable[hash_ipv6(addr, b)]; arec; arec = arec->next)
					if(arec->type == (type & ~0x1) &&
					   ...
					   comp_with_mask_sock(addr,
							       (struct sockaddr *) &arec->Mask.ipa.
							       addr, arec->Mask.ipa.bits)
	...
	if(orighost != NULL)
	{
		...
		for(arec = atable[0]; arec; arec = arec->next)
		{
			if(arec->type == (type & ~0x1) &&
			   ...
			   (type != CONF_CLIENT || !arec->auth_user
			    || (auth_user && match(arec->auth_user, auth_user))) && (type & 0x1
										     || match(arec->
											      username,
											      username)))
			{
 ...
 */
struct ConfItem *
find_address_conf(const char *host, const char *sockhost, const char *user, const char *notildeuser,
		  struct sockaddr *ip, int aftype, char *auth_user)
{
	...
	if(!
	   (iconf =
	    find_conf_by_address(host, sockhost, NULL, ip, CONF_CLIENT, aftype, user, auth_user)))
		return NULL;
	...
	if(IsConfDoSpoofIp(iconf))
	{
		...
		{
			*p = '\0';
			kconf = find_conf_by_address(p + 1, NULL, NULL, ip, CONF_KILL, aftype,
						     iconf->info.name, NULL);
			*p = '@';
		}
		else
			kconf = find_conf_by_address(iconf->info.name, NULL, NULL, ip, CONF_KILL,
						     aftype, vuser, NULL);
...
find_exact_conf_by_address(const char *address, int type, const char *username)
{
	...
	{
		if(arec->type == type && arec->masktype == masktype
		   && (arec->username == NULL
		       || username == NULL ? arec->username == username : !irccmp(arec->username,
										  username)))
		{
			if(masktype == HM_HOST)
			{
			...
			else
			{
				if(arec->Mask.ipa.bits == bits
				   && comp_with_mask_sock((struct sockaddr *) &arec->Mask.ipa.addr,
							  (struct sockaddr *) &addr, bits))
					return arec->aconf;
			}
		}
	}
 ...
 */
void
add_conf_by_address(const char *address, int type, const char *username, const char *auth_user,
		    struct ConfItem *aconf)
{
