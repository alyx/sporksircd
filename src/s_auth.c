...
auth_dns_callback(void *vptr, struct DNSReply *reply)
{
	...
	if(auth->client->localClient == NULL)
	{
		sendto_realops_snomask(SNO_GENERAL, L_ALL,
				       "auth_dns_callback(): auth->client->localClient (%s) is NULL",
...
start_auth_query(struct AuthRequest *auth)
{
	...
	{
		sendto_realops_snomask(SNO_GENERAL, L_ALL, "Can't allocate fd for auth on %s",
	...
	rb_connect_tcp(F, (struct sockaddr *) &destaddr, (struct sockaddr *) &localaddr,
		       GET_SS_LEN(&localaddr), auth_connect_callback, auth,
