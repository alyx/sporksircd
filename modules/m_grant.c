...
static int mo_ungrant(struct Client *client_p, struct Client *source_p, int parc, const char *parv[]);
static int me_ungrant(struct Client *client_p, struct Client *source_p, int parc, const char *parv[]);

static int do_grant(struct Client *source_p, struct Client *target_p, const char *new_privset, int deoper);

static char grantclass[] = "<grant>";
...
struct Message ungrant_msgtab = {
	"UNGRANT", 0, 0, 0, MFLG_SLOW,
	{mg_ignore, mg_not_oper, mg_ignore, mg_ignore, {me_ungrant, 2}, {mo_ungrant, 2}}
};

mapi_clist_av1 grant_clist[] = { &grant_msgtab, &ungrant_msgtab, NULL };

DECLARE_MODULE_AV1(grant, NULL, NULL, grant_clist, NULL, NULL, "SporksIRCD development team");
...
static int mo_ungrant(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if (MyClient(target_p))
		do_grant(source_p, target_p, parv[2], 1);
	else
	{
		sendto_one(target_p, ":%s ENCAP %s UNGRANT %s",
			   get_id(source_p, target_p), target_p->servptr->name,
...
static int me_ungrant(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	{
		sendto_one(source_p, ":%s NOTICE %s :You don't have an appropriate shared"
			   "block to ungrant privilege on this server.", me.name, source_p->name);
		return 0;
	}
