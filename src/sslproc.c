		...
		const char *ssl_dh_params)
{
	...
	if(ssld_path == NULL)
	{
		...
		{
			rb_snprintf(fullpath, sizeof(fullpath), "%s/libexec/sporksircd/ssld%s",
				    ConfigFileEntry.dpath, suffix);
			if(access(fullpath, X_OK) == -1)
			{
				ilog(L_MAIN, "Unable to execute ssld%s in %s/libexec/sporksircd or %s",
	...
	{
		ssl_ctl_t *ctl;
		if(rb_socketpair(AF_UNIX, SOCK_DGRAM, 0, &F1, &F2, "SSL/TLS handle passing socket")
		   == -1)
		{
			ilog(L_MAIN, "Unable to create ssld - rb_socketpair failed: %s",
			     strerror(errno));
			return started;
		}
...
ssl_process_certfp(ssl_ctl_t * ctl, ssl_ctl_buf_t * ctl_buf)
{
	...
	for(i = 0; i < RB_SSL_CERTFP_LEN; i++)
		rb_snprintf(certfp_string + 2 * i, 3, "%02x", certfp[i]);
...
start_zlib_session(void *data)
{
	...
	*buf = 'Z';
	...
	{
		sendto_realops_snomask(SNO_GENERAL, L_ALL, "Error creating zlib socketpair - %s",
				       strerror(errno));
