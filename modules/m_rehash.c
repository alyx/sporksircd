...
mapi_hlist_av1 rehash_hlist[] = {
	{"doing_rehash", &doing_rehash},
	{ NULL, NULL }
};

DECLARE_MODULE_AV1(rehash, NULL, NULL, rehash_clist, rehash_hlist, NULL, "SporksIRCD development team");

struct hash_commands
{
	const char *cmd;
	void (*handler) (struct Client * source_p, bool want_notice);
};

static void
rehash_bans_loc(struct Client *source_p, bool want_notice)
{
	if (want_notice)
...
static void
rehash_dns(struct Client *source_p, bool want_notice)
{
	if (want_notice)
...
static void
rehash_motd(struct Client *source_p, bool want_notice)
{
	if (want_notice)
...
static void
rehash_omotd(struct Client *source_p, bool want_notice)
{
	if (want_notice)
		sendto_realops_snomask(SNO_GENERAL, L_ALL,
...
static void
rehash_tklines(struct Client *source_p, bool want_notice)
{
...
static void
rehash_tdlines(struct Client *source_p, bool want_notice)
{
	struct ConfItem *aconf;
	rb_dlink_node *ptr, *next_ptr;
	int i;
...
static void
rehash_txlines(struct Client *source_p, bool want_notice)
{
	struct ConfItem *aconf;
	rb_dlink_node *ptr;
...
static void
rehash_tresvs(struct Client *source_p, bool want_notice)
{
	struct ConfItem *aconf;
	rb_dlink_node *ptr;
	rb_dlink_node *next_ptr;
...
static void
rehash_rejectcache(struct Client *source_p, bool want_notice)
{
	if (want_notice)
...
static void
rehash_throttles(struct Client *source_p, bool want_notice)
{
	if (want_notice)
...
static void
rehash_help(struct Client *source_p, bool want_notice)
{
	if (want_notice)
		sendto_realops_snomask(SNO_GENERAL, L_ALL,
...
static void
rehash_nickdelay(struct Client *source_p, bool want_notice)
{
	struct nd_entry *nd;
	rb_dlink_node *ptr;
...
static void
rehash_config(struct Client *source_p, bool want_notice)
{
	hook_data_int data;
	...
	/* XXX - Hmm... perhaps we should make this general purpose sometime...
	 * --Elizabeth */
	data.client = source_p;
	data.arg1 = "CONFIG";
	data.arg2 = want_notice;
	call_hook(doing_rehash, &data);
}
...
do_rehash(struct Client *source_p, const char *type)
{
	...
	if (type != NULL)
	{
		     ...
		     x++)
		{
			if(irccmp(type, rehash_commands[x].cmd) == 0)
			{
				...
				rehash_commands[x].handler(source_p, 1);
	...
	{
		sendto_one(source_p, form_str(RPL_REHASHING), me.name,
			   source_p->name, "CONFIG MOTD OMOTD");
		sendto_realops_snomask(SNO_GENERAL, L_ALL,
				       "%s is rehashing the server", get_oper_name(source_p));
		ilog(L_MAIN, "REHASH From %s[%s]", get_oper_name(source_p),
		     source_p->sockhost);

		rehash_config(source_p, 0);
		rehash_motd(source_p, 0);
		rehash_omotd(source_p, 0);
