...
#ifdef ABUSIVE_ROLEPLAY
static int m_fsay(struct Client *client_p, struct Client *source_p, int parc, const char *parv[]);
static int m_faction(struct Client *client_p, struct Client *source_p, int parc,
		     const char *parv[]);
#endif
...
static int m_displaymsg(struct Client *source_p, const char *channel, bool underline, bool action,
			const char *nick, const char *text);
static int me_roleplay(struct Client *client_p, struct Client *source_p, int parc,
		       const char *parv[]);
...
_modinit(void)
{
	...
	mymode = cmode_add('N', chm_simple, ISUPPORT_D);
	if(mymode == 0)
		return -1;
...
mapi_clist_av1 roleplay_clist[] = { &scene_msgtab, &ambiance_msgtab,
#ifdef ABUSIVE_ROLEPLAY
	&fsay_msgtab, &faction_msgtab,
#endif
	&npc_msgtab, &npca_msgtab, &roleplay_msgtab, NULL
};
...
static int
m_displaymsg(struct Client *source_p, const char *channel, bool underline, bool action,
	     const char *nick, const char *text)
{
	...
	if(!(chptr->mode.mode & mymode))
	{
		sendto_one_numeric(source_p, 573,
				   "%s :Roleplay commands are not enabled on this channel.",
				   chptr->chname);
		return 0;
	}

	rb_strlcpy(text2, text, BUFSIZE);
	if(!can_send(chptr, source_p, msptr, text2, COMMAND_PRIVMSG))
	{
	...
	if(!is_op_voiced(msptr) && !IsOper(source_p) && !add_channel_target(source_p, chptr))
	{
	...
	strip_unprintable(nick3, nick, NICKLEN + 1);

	if(underline)
		rb_snprintf(nick2, sizeof(nick2), "\x1F%s\x1F", nick3);
	else
		rb_snprintf(nick2, sizeof(nick2), "%s", nick3);
	...
	{
		sendto_one_numeric(source_p, 573,
				   "%s :No visible non-stripped characters in nick.",
				   chptr->chname);
		return 0;
	}

	if(action)
		rb_snprintf(text3, sizeof(text3), "\1ACTION %s\1", text2);
	else
		rb_snprintf(text3, sizeof(text3), "%s", text2);

	sendto_channel_local(ALL_MEMBERS, chptr,
			     ":%s!%s@npc.fakeuser.invalid PRIVMSG %s :%s",
			     nick2, source_p->name, channel, text2);
	sendto_match_servs(source_p, "*", CAP_ENCAP, NOCAPS,
			   "ENCAP * ROLEPLAY %s %s :%s", channel, nick2, text3);
	return 0;
}
...
me_roleplay(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	sendto_channel_local(ALL_MEMBERS, chptr,
			     ":%s!%s@npc.fakeuser.invalid PRIVMSG %s :%s",
			     parv[2], source_p->name, parv[1], parv[3]);
	return 0;
}
