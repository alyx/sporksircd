...
DECLARE_MODULE_AV1(capab, NULL, NULL, capab_clist, NULL, NULL, "SporksIRCD development team");
	...
	int parc, const char *parv[])
{
	...
	char *p;
	int newcaps = 0;
	char *newfullcaps = rb_strdup(parv[1]);
	...
	{
		if(!EmptyString(source_p->localClient->fullcaps))
			rb_free(source_p->localClient->fullcaps);
		
		source_p->localClient->caps = newcaps;
		source_p->localClient->fullcaps = newfullcaps;
	}
	else
	{
		...
		source_p->serv->caps = newcaps;
		source_p->serv->fullcaps = newfullcaps;
	}
