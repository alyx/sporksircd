...
/* BLEAH, this so doesn't belong here... but we don't have much choice */
typedef bool (*is_valid_item)(struct Channel *, char *);
...
/* ISUPPORT types */
#define ISUPPORT_A		0x01
#define ISUPPORT_B		0x02
#define ISUPPORT_C		0x03
#define ISUPPORT_D		0x04
#define ISUPPORT_PREFIX		0x05
#define ISUPPORT_INVALID	0xFF
...
#define CHFL_CHANOP     	0x0008	/* Channel operator */
#define CHFL_ADMIN		0x0010	/* cannot be kicked by ops */
#define CHFL_FOUNDER		0x0020	/* Owner of the channel */

#define CHFL_BANNED		0x0040  /* cached as banned */
#define CHFL_QUIETED		0x0080  /* cached as being +q victim */
#define ONLY_SERVERS		0x0100
#define	ONLY_OPERS		0x0120
...
#define IsMember(who, chan) ((who && who->user && \
	        find_channel_membership(chan, who)) ? YES : NO)
...
struct mode_list_t *allocate_list_item(const char *, const char *, const char *);
void free_list_item(struct mode_list_t *bptr);

extern void destroy_channel(struct Channel *);

extern int can_kick(struct Client *client_p, struct membership *source,
		    struct membership *target);
...
extern int is_banned(struct Channel *chptr, struct Client *who,
		     struct membership *msptr, const char *, const char *, char **);
extern int is_quieted(struct Channel *chptr, struct Client *who,
		     struct membership *msptr, const char *, const char *);
extern int can_join(struct Client *source_p, struct Channel *chptr, char *key, char **forward);
...
extern struct ChannelMode chmode_table[128];

extern int add_id(struct Client *source_p, struct Channel *chptr, const char *banid, const char *forward,
       rb_dlink_list * list, long mode_type, is_valid_item item_callback);

extern struct mode_list_t * del_id(struct Client *source_p, struct Channel *chptr, const char *banid, rb_dlink_list * list, long mode_type);


extern ExtbanFunc extban_table[128];

extern int extban_add(const char c, ExtbanFunc function);
extern int extban_delete(const char c);
