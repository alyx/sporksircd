...
static int mencap_test(struct Client *client_p, struct Client *source_p, int parc, const char *parv[]);
...
struct Message test_msgtab = {
   ...
   */
  {
    ...
    {mencap_test, 0},   /* function call for ENCAP (see TS6 specification), 0 parms required */
...
DECLARE_MODULE_AV1(
			  ...
			  test_hfnlist,
			  /* And finally the version number of this module (official sporksircd
			   * modules have this set to SporksIRCD development team). */
			  "SporksIRCD development team");
 ...
 */
static int
mencap_test(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	if(parc < 2)
	{
		sendto_one_notice(source_p, ":You are a server using ENCAP, and sent no parameters");
	}
	else
	{
		sendto_one_notice(source_p, ":You are a server using ENCAP, and sent parameters: %s", parv[1]);
	}
	return 0;
}
