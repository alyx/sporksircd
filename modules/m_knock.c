...
DECLARE_MODULE_AV1(knock, modinit, modfini, knock_clist, NULL, NULL, "SporksIRCD development team");

static int
modinit(void)
{
	add_capability("KNOCK", CAP_KNOCK, YES, NO);
	return 0;
}
...
m_knock(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(ConfigChannel.use_knock)
		sendto_channel_local(chptr->mode.mode & MODE_FREEINVITE ? ALL_MEMBERS : ONLY_HALFOPSANDUP,
