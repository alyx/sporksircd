...
ExtbanFunc extban_table[128] = { NULL };

int
extban_add(const char c, ExtbanFunc function)
{
	if(extban_table[(uint8_t) c] != NULL)
		return -1;

	extban_table[(uint8_t) c] = function;
...
int
extban_delete(const char c)
{
	if(extban_table[(uint8_t) c] == NULL)
		return -1;

	extban_table[(uint8_t) c] = NULL;
