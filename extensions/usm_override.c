/*
 * oper-override for SporksIRCD.
 * Substantially derived from the override module in charybdis.
 *
...
static void handle_can_kick(hook_data_channel_approval *data);

static int conf_expire_time = 300;
static int conf_make_immune = NO;
...
mapi_hfn_list_av1 override_hfnlist[] = {
	...
	{ "can_kick", (hookfn) handle_can_kick },
	{ NULL, NULL }
};

static struct ConfEntry conf_override_table[] =
{
	{ "expire_time", CF_TIME,  NULL, 0, &conf_expire_time },
	{ "make_immune", CF_YESNO, NULL, 0, &conf_make_immune },
};
...
update_session_deadline(struct Client *source_p, struct OverrideSession *session_p)
{
	...
	session_p->deadline = rb_current_time() + conf_expire_time;
...
check_umode_change(void *vdata)
{
	...
	if (source_p->umodes & user_modes['p'])
	{
		...
		sendto_realops_snomask(SNO_GENERAL, L_NETWIDE, "%s has enabled oper-override on themselves (+p)",
	...
	else if (!(source_p->umodes & user_modes['p']))
	{
		...
		RB_DLINK_FOREACH_SAFE(n, tn, overriding_opers.head)
		{
			...
			sendto_realops_snomask(SNO_GENERAL, L_NETWIDE, "%s has disabled oper-override on themselves (-p)",
...
hack_channel_access(void *vdata)
{
	...
	if (data->approved == CHFL_FOUNDER)
		return;

	if (data->client->umodes & user_modes['p'])
	{
		update_session_deadline(data->client, NULL);
		data->approved = CHFL_FOUNDER;

		sendto_realops_snomask(SNO_GENERAL, L_NETWIDE, "%s is using oper-override on %s (overriding access)",
...
hack_can_join(void *vdata)
{
	...
	if (data->client->umodes & user_modes['p'])
	{
		...
		sendto_realops_snomask(SNO_GENERAL, L_NETWIDE, "%s is using oper-override on %s (overriding join)",
				       get_oper_name(data->client), data->chptr->chname);
	}
}
...
static void
handle_can_kick(hook_data_channel_approval *data)
{
	struct Client *source_p = data->client;
	...
	if (data->target->umodes & user_modes['p'])
	{
		sendto_one_numeric(source_p, ERR_ISCHANSERVICE,
				   "%s %s :User is immune from kick/remove.",
				   target_p->name, chptr->chname);
		sendto_realops_snomask(SNO_GENERAL, L_NETWIDE, "Attempt to KICK/REMOVE %s on %s in %s (operator is immune)",
				       target_p->name, source_p->name, chptr->chname);
		data->approved = 0;
	}
}
...
_modinit(void)
{
	...
	add_top_conf("override", NULL, NULL, conf_override_table);

	expire_override_deadlines_ev = rb_event_add("expire_override_deadlines", expire_override_deadlines, NULL, 15);
