 ...
 */
rb_dlink_list caplist = { NULL, NULL, 0 };

void
add_capability(const char *capability, int flag, bool add_default, bool required)
{
	struct Capability *cap = rb_malloc(sizeof(struct Capability));
	cap->name = rb_strdup(capability);
	cap->cap = flag;
	cap->required = required;

	if(add_default)
		default_server_capabs |= flag;

	rb_dlinkAdd(cap, &cap->node, &caplist);

	capab_update();
}

void
delete_capability(const char *capability)
{
	rb_dlink_node *ptr, *nptr;

	RB_DLINK_FOREACH_SAFE(ptr, nptr, caplist.head)
	{
		struct Capability *cap = ptr->data;
		if (cap->cap != 0)
		{
			if(irccmp(cap->name, capability) == 0)
			{
				default_server_capabs &= ~(cap->cap);
				rb_dlinkDelete(ptr, &caplist);
				rb_free(cap->name);
				rb_free(cap);
			}
		}
 ...
 */
int
hunt_server(struct Client *client_p, struct Client *source_p, const char *command, int server,
	    int parc, const char *parv[])
{
	 ...
	 */
	if(parc <= server || EmptyString(parv[server]) || match(parv[server], me.name)
	...
	if(target_p)
	{
		...
		sendto_one(target_p, command, get_id(source_p, target_p), parv[1], parv[2], parv[3],
...
try_connections(void *unused)
{
	 ...
	 *   -- adrian
	 */
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "Connection to %s activated", server_p->name);

	serv_connect(server_p, 0);
}

int
match_all_servers(const char *pattern)
{
	rb_dlink_node *ptr;
	...
	RB_DLINK_FOREACH(ptr, global_serv_list.head)
	{
		...
		{
			if(!match(pattern, client_p->name))
				return NO;
		}
	}
	...
	RB_DLINK_FOREACH(ptr, server_conf_list.head)
	{
		...
		if(match(tmp_p->host, client_p->host) || match(tmp_p->host, client_p->sockhost))
		{
			...
			if(tmp_p->passwd)
			{
				...
				{
					if(!strcmp(tmp_p->passwd, rb_crypt(client_p->localClient->passwd, tmp_p->passwd)))
					{
						server_p = tmp_p;

						/* Warn about DES usage --Elizabeth */
						if(tmp_p->passwd[0] != '$' && (tmp_p->passwd[2] != '$' || tmp_p->passwd[3] != '$'))
						{
							sendto_realops_snomask(SNO_GENERAL, L_NETWIDE,
									       "Warning: Server block %s may be using a DES encrypted password, "
									       "which is considered insecure. Consider switching to SHA256.",
									       name);
						}
			...
			{
				if(!client_p->certfp
				   || strcasecmp(tmp_p->certfp, client_p->certfp) != 0)
					continue;
			}
...
static inline void
build_capab_list(char *s, int cap_can_send)
{
	rb_dlink_node *ptr;

	RB_DLINK_FOREACH(ptr, caplist.head)
	{
		struct Capability *cap = ptr->data;
		if(cap->cap & (cap_can_send | default_server_capabs))
		{
			s += rb_sprintf(s, "%s ", cap->name);
		}
	}
...
send_capabilities(struct Client *client_p, int cap_can_send)
{
	...
	build_capab_list(msgbuf, cap_can_send);
...
capab_update(void)
{
	...
	RB_DLINK_FOREACH(ptr, global_serv_list.head)
	{
		...
		build_capab_list(msgbuf, default_server_capabs |
					 (ServerConfCompressed(target_p) ? CAP_ZIP_SUPPORTED : 0) |
					 (ServerConfTb(target_p) ? CAP_TB : 0));
		sendto_one(target_p, ":%s ENCAP * GCAP :%s", me.id, msgbuf);
	}
}
...
burst_ban(struct Client *client_p)
{
	...
	RB_DLINK_FOREACH(ptr, prop_bans.head)
	{
		...
		if(aconf->flags & CONF_FLAGS_MYOPER)
		{
			...
			p = strrchr(operbuf, '{');
			if(p != NULL && operbuf + sizeof operbuf - p > (ptrdiff_t) (melen + 2))
			{
		...
		sendto_one(client_p, ":%s BAN %s %s %s %lu %d %d %s :%s%s%s",
			   ...
			   (int) (aconf->lifetime - aconf->created), oper, aconf->passwd,
			   aconf->spasswd ? "|" : "", aconf->spasswd ? aconf->spasswd : "");
	}
}
 ...
 * output	-
 * side effects - client is sent a list of +b/+e/+I/whatever modes
 */
void
burst_modes_TS6(struct Client *client_p, struct Channel *chptr, rb_dlink_list * list, char flag)
{
	...
	/* XXX should not *always* use BMASK blindly... */
	cur_len = mlen =
		rb_sprintf(buf, ":%s BMASK %ld %s %c :", me.id, (long) chptr->channelts,
			   chptr->chname, flag);
	t = buf + mlen;
	...
	RB_DLINK_FOREACH(ptr, list->head)
	{
		...
		rb_sprintf(t, "%s ", listptr->maskstr);
		t += tlen;
		cur_len += tlen;
	}
...
burst_TS6(struct Client *client_p)
{
	...
	RB_DLINK_FOREACH(ptr, global_client_list.head)
	{
		...
		if(IsCapable(client_p, CAP_EUID))
			sendto_one(client_p, ":%s EUID %s %d %ld %s %s %s %s %s %s %s :%s",
				   ...
				   target_p->id, IsDynSpoof(target_p) ? target_p->orighost : "*",
			...
			sendto_one(client_p, ":%s UID %s %d %ld %s %s %s %s %s :%s",
				   target_p->servptr->id, target_p->name,
				   target_p->hopcount + 1,
				   (long) target_p->tsinfo, ubuf, target_p->username,
				   target_p->host, IsIPSpoof(target_p) ? "0" : target_p->sockhost,
				   target_p->id, target_p->info);

		if(!EmptyString(target_p->certfp) && IsCapable(client_p, CAP_ENCAP))
			sendto_one(client_p, ":%s ENCAP * CERTFP :%s", use_id(target_p),
				   target_p->certfp);

		if(!IsCapable(client_p, CAP_EUID) && IsCapable(client_p, CAP_ENCAP))
		{
			if(IsDynSpoof(target_p))
				sendto_one(client_p, ":%s ENCAP * REALHOST %s", use_id(target_p),
					   target_p->orighost);
			if(!EmptyString(target_p->user->suser))
				sendto_one(client_p, ":%s ENCAP * LOGIN %s", use_id(target_p),
					   target_p->user->suser);
		}

		RB_DICTIONARY_FOREACH(md, &iter, target_p->user->metadata)
		{
		...
		if(ConfigFileEntry.burst_away && !EmptyString(target_p->user->away))
			sendto_one(client_p, ":%s AWAY :%s", use_id(target_p),
				   target_p->user->away);
	...
	RB_DLINK_FOREACH(ptr, global_channel_list.head)
	{
		...
		cur_len = mlen =
			rb_sprintf(buf, ":%s SJOIN %ld %s %s :", me.id, (long) chptr->channelts,
				   chptr->chname, channel_modes(chptr, client_p));
		...
		for(i = 0; i < 128; i++)
		{
			...
			clist = get_channel_list(chptr, mode->c);
			if(rb_dlink_list_length(clist) > 0)
				burst_modes_TS6(client_p, chptr, clist, mode->c);
		}
		...
		if(IsCapable(client_p, CAP_MLOCK))
			sendto_one(client_p, ":%s MLOCK %ld %s :%s", me.id, (long) chptr->channelts,
				   chptr->chname,
...
server_estab(struct Client *client_p)
{
	...
	if(IsUnknown(client_p))
	{
		...
		sendto_one(client_p, "PASS %s TS %d :%s",
			   EmptyString(server_p->spasswd) ? "*" : server_p->spasswd, TS_CURRENT,
			   me.id);
	...
	/* Send the supported modes
	 * NOTE - we may not have CAPAB stuff from remote yet, not a problem in
	 * practise though I suspect as most servers send CAPAB before SERVER,
	 * making the problem a bit academic --Elizabeth
	 */
	if(IsCapable(client_p, CAP_MS))
		sendto_one(client_p, "MS %s %s %s %s %s", cmodes_a, cmodes_b, cmodes_c,
			   cmodes_d, umodebuf);
	...
	client_p->serv->nameinfo =
		scache_connect(client_p->name, client_p->info, IsHidden(client_p));
	...
	if((rb_dlink_list_length(&lclient_list) + rb_dlink_list_length(&serv_list)) >
	   (unsigned long) MaxConnectionCount)
		MaxConnectionCount =
			rb_dlink_list_length(&lclient_list) + rb_dlink_list_length(&serv_list);

	/* Show the real host/IP to admins */
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "Link with %s established: (%s) link",
			       client_p->name, show_capabilities(client_p));

	ilog(L_SERVER, "Link with %s established: (%s) link", log_client_name(client_p, SHOW_IP),
	...
	RB_DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		{
			sendto_one(target_p, ":%s SID %s 2 %s :%s%s", me.id, client_p->name,
				   client_p->id, IsHidden(client_p) ? "(H) " : "", client_p->info);

			if(IsCapable(target_p, CAP_ENCAP) && !EmptyString(client_p->serv->fullcaps))
				sendto_one(target_p, ":%s ENCAP * GCAP :%s", client_p->id,
					   client_p->serv->fullcaps);
		}
		else
		{
			sendto_one(target_p, ":%s SERVER %s 2 :%s%s", me.name, client_p->name,
				   IsHidden(client_p) ? "(H) " : "", client_p->info);

			if(IsCapable(target_p, CAP_ENCAP) && !EmptyString(client_p->serv->fullcaps))
				sendto_one(target_p, ":%s ENCAP * GCAP :%s", client_p->name,
					   client_p->serv->fullcaps);
		}
	}
	...
	RB_DLINK_FOREACH(ptr, global_serv_list.head)
	{
		...
		if(has_id(client_p) && has_id(target_p))
			sendto_one(client_p, ":%s SID %s %d %s :%s%s", target_p->servptr->id,
				   target_p->name, target_p->hopcount + 1, target_p->id,
				   IsHidden(target_p) ? "(H) " : "", target_p->info);
		else
			sendto_one(client_p, ":%s SERVER %s %d :%s%s", target_p->servptr->name,
				   target_p->name, target_p->hopcount + 1,
				   IsHidden(target_p) ? "(H) " : "", target_p->info);

		if(IsCapable(client_p, CAP_ENCAP) && !EmptyString(target_p->serv->fullcaps))
			sendto_one(client_p, ":%s ENCAP * GCAP :%s", get_id(target_p, client_p),
...
serv_connect_resolved(struct Client *client_p)
{
	...
	{
		/* log */
		ilog(L_SERVER, "Connecting to %s[%s] port %d (%s)", client_p->name,
		     client_p->sockhost, port,
		     server_p->aftype == AF_INET6 ? "IPv6" : (server_p->aftype ==
							      AF_INET ? "IPv4" : "?"));

		if(ServerConfSSL(server_p))
		{
			rb_connect_tcp(client_p->localClient->F,
				       (struct sockaddr *) &client_p->localClient->ip, NULL, 0,
	...
	ilog(L_SERVER, "Connecting to %s[%s] port %d (%s) (vhost %s)", client_p->name,
	     client_p->sockhost, port,
	     server_p->aftype == AF_INET6 ? "IPv6" : (server_p->aftype == AF_INET ? "IPv4" : "?"),
	     vhoststr);
...
serv_connect_dns_callback(void *vptr, struct DNSReply *reply)
{
	...
	{
		sendto_realops_snomask(SNO_GENERAL, is_remote_connect(client_p) ? L_NETWIDE : L_ALL,
				       "Cannot resolve hostname for %s", client_p->name);
	...
	rb_inet_ntop_sock((struct sockaddr *) &client_p->localClient->ip, client_p->sockhost,
			  sizeof client_p->sockhost);
...
serv_connect(struct server_conf *server_p, struct Client *by)
{
	...
	{
		sendto_realops_snomask(SNO_GENERAL, L_ALL, "Server %s already present from %s",
				       server_p->name, client_p->name);
		if(by && IsPerson(by) && !MyClient(by))
			sendto_one_notice(by, ":Server %s already present from %s", server_p->name,
					  client_p->name);
		return 0;
	}
	...
	if(rb_inet_pton_sock(server_p->host, (struct sockaddr *) &theiripnum) > 0)
	{
		...
		if(theiripnum.ss_family == AF_INET6)
			((struct sockaddr_in6 *) &client_p->localClient->ip)->sin6_port =
				htons(server_p->port);
		else
			((struct sockaddr_in *) &client_p->localClient->ip)->sin_port =
				htons(server_p->port);
	...
	else
	{
		...
		gethost_byname_type(server_p->host, client_p->localClient->dnsquery,
				    server_p->aftype == AF_INET6 ? T_AAAA : T_A);
		return 1;
	}
}
...
serv_connect_callback(rb_fde_t * F, int status, void *data)
{
	...
	if(status != RB_OK)
	{
		...
		if(status == RB_ERR_TIMEOUT)
		{
			sendto_realops_snomask(SNO_GENERAL,
					       ...
					       "255.255.255.255", rb_errstr(status));
			ilog(L_SERVER, "Error connecting to %s[%s]: %s", client_p->name,
			     client_p->sockhost, rb_errstr(status));
		}
		else
		{
			...
			sendto_realops_snomask(SNO_GENERAL,
					       ...
					       client_p->name, "255.255.255.255", rb_errstr(status),
					       errstr);
			ilog(L_SERVER, "Error connecting to %s[%s]: %s (%s)", client_p->name,
			     client_p->sockhost, rb_errstr(status), errstr);
		}
	...
	sendto_one(client_p, "SERVER %s 1 :%s%s", me.name, ConfigServerHide.hidden ? "(H) " : "",
		   me.info);
