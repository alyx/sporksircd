...
/* Modes we're allowed to orphan
 * XXX we should do something about this... this gets hard to maintain */
static const char orphan_only[] = "EFIJLMPQefgjqrz";
...
init_s_conf(void)
{
	...
	rb_event_addish("expire_temp_klines_hour", reorganise_temp_kd, &temp_klines[TEMP_HOUR],
			3600);
	rb_event_addish("expire_temp_dlines_hour", reorganise_temp_kd, &temp_dlines[TEMP_HOUR],
			3600);
	rb_event_addish("expire_temp_klines_day", reorganise_temp_kd, &temp_klines[TEMP_DAY],
			86400);
	rb_event_addish("expire_temp_dlines_day", reorganise_temp_kd, &temp_dlines[TEMP_DAY],
			86400);
	rb_event_addish("expire_temp_klines_week", reorganise_temp_kd, &temp_klines[TEMP_WEEK],
			604800);
	rb_event_addish("expire_temp_dlines_week", reorganise_temp_kd, &temp_dlines[TEMP_WEEK],
			604800);
}
...
check_client(struct Client *client_p, struct Client *source_p, const char *username)
{
	...
	case TOO_MANY_LOCAL:
		...
		sendto_realops_snomask(SNO_FULL, L_NETWIDE,
				       ...
				       source_p->username, show_ip(NULL, source_p)
				       && !IsIPSpoof(source_p) ? source_p->sockhost : source_p->
				       host);

		ilog(L_FUSER, "Too many local connections from %s!%s%s@%s", source_p->name,
	...
	case I_LINE_FULL:
		sendto_realops_snomask(SNO_FULL, L_NETWIDE,
				       ...
				       source_p->name, IsGotId(source_p) ? "" : "~",
				       source_p->username, source_p->host, show_ip(NULL, source_p)
				       && !IsIPSpoof(source_p) ? source_p->
				       sockhost : "255.255.255.255");

		ilog(L_FUSER, "Too many connections from %s!%s%s@%s.", source_p->name,
		     IsGotId(source_p) ? "" : "~", source_p->username, source_p->sockhost);
	...
	case NOT_AUTHORISED:
		{
			...
			if(source_p->localClient->ip.ss_family == AF_INET6)
				port = ntohs(((struct sockaddr_in6 *) &source_p->localClient->
					      listener->addr)->sin6_port);
			else
				port = ntohs(((struct sockaddr_in *) &source_p->localClient->
					      listener->addr)->sin_port);
			...
			ilog(L_FUSER,
			     ...
			     source_p->name, IsGotId(source_p) ? "" : "~", source_p->username,
...
verify_access(struct Client *client_p, const char *username)
{
	...
	if(aconf->status & CONF_CLIENT)
	{
		...
		if(IsConfDoSpoofIp(aconf))
		{
			...
			{
				sendto_realops_snomask(SNO_GENERAL, L_ALL, "%s spoofing: %s as %s",
						       client_p->name, show_ip(NULL,
									       client_p) ?
						       client_p->host : aconf->info.name,
						       aconf->info.name);
			}
	...
	else if(aconf->status & CONF_KILL)
	{
		if(ConfigFileEntry.kline_with_reason)
			sendto_one(client_p, form_str(ERR_YOUREBANNEDCREEP), me.name,
				   client_p->name, get_user_ban_reason(aconf));
...
attach_conf(struct Client *client_p, struct ConfItem *aconf)
{
	...
	if((aconf->status & CONF_CLIENT) && ConfCurrUsers(aconf) >= ConfMaxUsers(aconf) &&
	   ConfMaxUsers(aconf) > 0)
	{
		if(!IsConfExemptLimits(aconf))
		{
			return (I_LINE_FULL);
...
rehash(int sig)
{
	...
	/* Hmm... we better regen isupport eh? */
	if(isupport_regen)
		cache_isupport();
...
set_default_conf(void)
{
	...
	ConfigChannel.founder_on_channel_create = NO;
	...
	ConfigFileEntry.client_flood_burst_rate = 40;
	...
	if(!alias_dict)
		alias_dict = rb_dictionary_create(strcasecmp);
}
...
validate_conf(void)
{
	...
	if(!valid_hostname(ConfigFileEntry.default_operhost) &&
	   !EmptyString(ConfigFileEntry.default_operhost))
	{
...
valid_wild_card(const char *luser, const char *lhost)
{
	...
	if((p = strrchr(lhost, '/')) != NULL && IsDigit(p[1]))
	{
		...
		if(bitlen > 0 &&
		   bitlen >= (strchr(lhost, ':') ? 4 * (ConfigFileEntry.min_nonwildcard - nonwild) : 6 - 2 * nonwild))
			return 1;
	}
	else
	{
...
find_prop_ban(unsigned int status, const char *user, const char *host)
{
	...
	RB_DLINK_FOREACH(ptr, prop_bans.head)
	{
		...
		if((aconf->status & ~CONF_ILLEGAL) == status &&
		   (!user || !aconf->user || !irccmp(aconf->user, user)) &&
...
deactivate_conf(struct ConfItem *aconf, rb_dlink_node * ptr)
{
	...
	{
	case CONF_KILL:
		if(aconf->lifetime == 0 && aconf->flags & CONF_FLAGS_TEMPORARY)
...
expire_prop_bans(void *list)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, ((rb_dlink_list *) list)->head)
	{
		...
		if(aconf->lifetime <= rb_current_time() ||
		   (aconf->hold <= rb_current_time() && !(aconf->status & CONF_ILLEGAL)))
		{
			/* Alert opers that a TKline expired - Hwy */
			/* XXX show what type of ban it is */
			if(ConfigFileEntry.tkline_expire_notices && !(aconf->status & CONF_ILLEGAL))
				sendto_realops_snomask(SNO_GENERAL, L_ALL,
...
reorganise_temp_kd(void *list)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, ((rb_dlink_list *) list)->head)
	{
		...
		{
			rb_dlinkMoveNode(ptr, list,
					 (aconf->status == CONF_KILL) ? &temp_klines[TEMP_MIN] : &temp_dlines[TEMP_MIN]);
			aconf->port = TEMP_MIN;
		}
		else if(aconf->port > TEMP_HOUR)
		{
			...
			{
				rb_dlinkMoveNode(ptr, list,
						 (aconf->status == CONF_KILL) ? &temp_klines[TEMP_HOUR] : &temp_dlines[TEMP_HOUR]);
				aconf->port = TEMP_HOUR;
			}
			...
			{
				rb_dlinkMoveNode(ptr, list,
						 (aconf->status == CONF_KILL) ? &temp_klines[TEMP_DAY] : &temp_dlines[TEMP_DAY]);
				aconf->port = TEMP_DAY;
			}
		}
	}
...
get_oper_name(struct Client *client_p)
{
	...
	if(MyOper(client_p))
	{
		rb_snprintf(buffer, sizeof(buffer), "%s!%s@%s{%s}", client_p->name,
			    client_p->username, client_p->host, client_p->localClient->opername);
		return buffer;
	}

	rb_snprintf(buffer, sizeof(buffer), "%s!%s@%s{%s}", client_p->name, client_p->username,
 ...
 */
void
get_printable_conf(struct ConfItem *aconf, char **name, char **host, char **pass, char **user,
		   int *port, char **classname)
{
...
get_user_ban_reason(struct ConfItem *aconf)
{
	...
	if(aconf->flags & CONF_FLAGS_TEMPORARY &&
	   (aconf->status == CONF_KILL || aconf->status == CONF_DLINE))
		rb_snprintf(reasonbuf, sizeof reasonbuf, "Temporary %c-line %d min. - ",
	...
	if(aconf->created)
	{
		...
		rb_strlcat(reasonbuf, smalldate(aconf->created), sizeof reasonbuf);
		rb_strlcat(reasonbuf, ")", sizeof reasonbuf);
	}
	return reasonbuf;
}
...
void
get_printable_kline(struct Client *source_p, struct ConfItem *aconf, char **host, char **reason,
	...
	*reason = get_user_ban_reason(aconf);
	...
	else
	{
		rb_snprintf(operreasonbuf, sizeof operreasonbuf, "%s%s(%s)",
			    ...
			    EmptyString(aconf->spasswd) ? "" : " ", aconf->info.oper);
 ...
 * output       - none
 * side effects - read all conf files needed, ircd.conf etc.
 */
void
 ...
 */
static void
free_alias_cb(struct rb_dictionaryElement *ptr, void *unused)
{
...
conf_add_class_to_conf(struct ConfItem *aconf)
{
	...
	if(ClassPtr(aconf) == default_class)
	{
		if(aconf->status == CONF_CLIENT)
		{
			conf_report_error("Using default class for missing class \"%s\" in auth{} for %s@%s",
...
yyerror(const char *msg)
{
	...
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "\"%s\", line %d: %s at '%s'", conffilebuf,
