...
show_lusers(struct Client *source_p)
{
	...
	if(rb_dlink_list_length(&oper_list) > 0)
		sendto_one_numeric(source_p, RPL_LUSEROP, form_str(RPL_LUSEROP),
				   rb_dlink_list_length(&oper_list));

	if(rb_dlink_list_length(&unknown_list) > 0)
		sendto_one_numeric(source_p, RPL_LUSERUNKNOWN, form_str(RPL_LUSERUNKNOWN),
				   rb_dlink_list_length(&unknown_list));

	if(rb_dlink_list_length(&global_channel_list) > 0)
		sendto_one_numeric(source_p, RPL_LUSERCHANNELS, form_str(RPL_LUSERCHANNELS),
	...
	sendto_one_numeric(source_p, RPL_GLOBALUSERS, form_str(RPL_GLOBALUSERS), Count.total,
			   Count.max_tot, Count.total, Count.max_tot);

	sendto_one_numeric(source_p, RPL_STATSCONN, form_str(RPL_STATSCONN), MaxConnectionCount,
...
register_local_user(struct Client *client_p, struct Client *source_p, const char *username)
{
	...
	if(!valid_hostname(source_p->host))
	{
		sendto_one_notice(source_p,
				  ":*** Notice -- You have an illegal character in your hostname");
	...
	if(!EmptyString(aconf->passwd))
	{
		...
		/* Report possible DES usage --Elizabeth */
		if(IsConfEncrypted(aconf) && aconf->passwd[0] != '$'
		   && (aconf->passwd[2] != '$' || aconf->passwd[3] != '$'))
		{
			sendto_realops_snomask(SNO_GENERAL, L_NETWIDE,
					       "Warning: Auth block matching %s (%s@%s) may be using a DES encrypted password, "
		...
		if(source_p->localClient->passwd)
		{
			memset(source_p->localClient->passwd, 0,
			       strlen(source_p->localClient->passwd));
	 ...
	 */
	/* Except "F:" clients */
	if(rb_dlink_list_length(&lclient_list) >= (unsigned long) GlobalSetOptions.maxclients &&
	   !IsConfExemptLimits(aconf))
	{
		sendto_realops_snomask(SNO_FULL, L_ALL, "Too many clients, rejecting %s[%s].",
	...
	if(!IsExemptKline(source_p) && (xconf = find_xline(source_p->info, 1)) != NULL)
	{
		ServerStats.is_ref++;
	...
	if(source_p->preClient->dnsbl_listed != NULL)
	{
		if(IsExemptKline(source_p) || IsConfExemptDNSBL(aconf))
			sendto_one_notice(source_p,
					  ":*** Your IP address %s is listed in %s, but you are exempt",
					  source_p->sockhost,
		...
		else
		{
			...
			substitution_append_var(&varlist, "dnsbl-host",
						source_p->preClient->dnsbl_listed->host);
			substitution_append_var(&varlist, "network-name", ServerInfo.network_name);

			ServerStats.is_ref++;

			sendto_one(source_p, form_str(ERR_YOUREBANNEDCREEP), me.name,
				   source_p->name,
				   substitution_parse(source_p->preClient->dnsbl_listed->
						      reject_reason, &varlist));
	...
	{
		sendto_realops_snomask(SNO_REJ, L_ALL, "Invalid username: %s (%s@%s)",
				       source_p->name, source_p->username, source_p->host);
		ServerStats.is_ref++;
		sendto_one_notice(source_p,
	...
	source_p->umodes |=
		ConfigFileEntry.default_umodes & ~ConfigFileEntry.
		oper_only_umodes & ~orphaned_umodes;
	...
	sendto_realops_snomask(SNO_CCONN, L_ALL,
			       ...
			       show_ip(NULL, source_p) ? ipaddr : "255.255.255.255", get_client_class(source_p),
			       source_p->info);
	...
	if(rb_dlink_list_length(&lclient_list) > (unsigned long) Count.max_loc)
	{
		...
		if(!(Count.max_loc % 10))
			sendto_realops_snomask(SNO_GENERAL, L_ALL, "New Max Local Clients: %d",
					       Count.max_loc);
	}
 ...
 */
int
introduce_client(struct Client *client_p, struct Client *source_p, struct User *user,
		 const char *nick, int use_euid)
{
	static char ubuf[12];
	...
	if(use_euid)
		sendto_server(client_p, NULL, CAP_EUID | CAP_TS6, NOCAPS,
			      ...
			      source_p->id, IsDynSpoof(source_p) ? source_p->orighost : "*",
	...
	sendto_server(client_p, NULL, CAP_TS6, use_euid ? CAP_EUID : NOCAPS,
		      ...
		      source_p->servptr->id, nick,
		      source_p->hopcount + 1, (long) source_p->tsinfo, ubuf, source_p->username,
		      source_p->host, IsIPSpoof(source_p) ? "0" : sockhost, source_p->id,
		      source_p->info);

	if(!EmptyString(source_p->certfp))
		sendto_server(client_p, NULL, CAP_TS6, NOCAPS, ":%s ENCAP * CERTFP :%s",
			      use_id(source_p), source_p->certfp);

	if(IsDynSpoof(source_p))
	{
		sendto_server(client_p, NULL, CAP_TS6, use_euid ? CAP_EUID : NOCAPS,
			      ":%s ENCAP * REALHOST %s", use_id(source_p), source_p->orighost);
	}

	if(!EmptyString(source_p->user->suser))
	{
		sendto_server(client_p, NULL, CAP_TS6, use_euid ? CAP_EUID : NOCAPS,
			      ":%s ENCAP * LOGIN %s", use_id(source_p), source_p->user->suser);
	}
...
show_other_user_mode(struct Client *source_p, struct Client *target_p)
{
	...
	*m = '\0';
	...
	if(MyConnect(target_p) && target_p->snomask != 0)
		sendto_one_notice(source_p, ":Modes for %s are %s %s", target_p->name, buf,
				  construct_snobuf(target_p->snomask));
	else
		sendto_one_notice(source_p, ":Modes for %s are %s", target_p->name, buf);
}
...
user_mode(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	int what, setflags;
	bool badflag = NO;	/* Only send one bad flag notice */
	bool showsnomask = NO;
	...
	if((target_p =
	    MyClient(source_p) ? find_named_person(parv[1]) : find_person(parv[1])) == NULL)
	{
		if(MyConnect(source_p))
	...
	{
		sendto_realops_snomask(SNO_GENERAL, L_ADMIN, "*** Mode for User %s from %s",
	...
	for(pm = parv[2]; *pm; pm++)
		...
		case 'o':
			...
			else
			{
				...
				if(MyConnect(source_p))
				{
					...
					if(!(source_p->umodes & UMODE_SERVNOTICE)
					   && source_p->snomask != 0)
					{
						source_p->snomask = 0;
						showsnomask = YES;
		...
		case 's':
			if(MyConnect(source_p))
			{
				...
				if(what == MODE_ADD)
				{
					if(parc > 3)
						source_p->snomask =
							parse_snobuf_to_mask(source_p->snomask,
									     parv[3]);
					else
		...
		default:
			...
			if((flag = user_modes[(unsigned char) *pm]))
			{
				if(MyConnect(source_p)
				   &&
				   ((!IsOper(source_p) && (ConfigFileEntry.oper_only_umodes & flag))
...
send_umode_out(struct Client *client_p, struct Client *source_p, int old)
{
	...
	RB_DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		{
			sendto_one(target_p, ":%s MODE %s :%s", get_id(source_p, target_p),
...
user_welcome(struct Client *source_p)
{
	...
	sendto_one_numeric(source_p, RPL_MYINFO, form_str(RPL_MYINFO), me.name, ircd_version,
			   umodebuf, cmodes_all, cmodes_params);
	...
	{
		sendto_one_notice(source_p, ":*** Notice -- motd was last changed at %s",
				  user_motd_changed);
		sendto_one_notice(source_p,
				  ":*** Notice -- Please read the motd if you haven't read it");

		sendto_one(source_p, form_str(RPL_MOTDSTART), me.name, source_p->name, me.name);

		sendto_one(source_p, form_str(RPL_MOTD), me.name, source_p->name,
...
oper_up(struct Client *source_p, struct oper_conf *oper_p)
{
	...
	{
		if(oper_p->vhost)
			change_nick_user_host(source_p, source_p->name, source_p->username,
					      oper_p->vhost, 0, "Changing host");
		else
			change_nick_user_host(source_p, source_p->name, source_p->username,
					      GlobalSetOptions.operhost, 0, "Changing host");

		sendto_one_numeric(source_p, RPL_HOSTHIDDEN,
				   "%s :is now your hidden host (set by %s)", source_p->host,
	...
	/* Ok, we have the privset, if it has an operstring and the oper hasn't
	 * overridden it, we'll set it here --Elizabeth
	 */
	if(source_p->localClient->privset->operstring && EmptyString(oper_p->operstring))
		user_metadata_add(source_p, "OPERSTRING",
				  source_p->localClient->privset->operstring, 1);
	...
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "%s (%s!%s@%s) is now an operator", oper_p->name,
			       source_p->name, source_p->username, source_p->host);
...
find_umode_slot(void)
{
	...
	for(my_umode = 1; my_umode && (all_umodes & my_umode); my_umode <<= 1);
		...
		if(prev_user_modes[i] != 0 && prev_user_modes[i] != user_modes[i])
		{
			if(user_modes[i] == 0)
			{
				...
				sendto_realops_snomask(SNO_DEBUG, L_ALL,
						       "Umode +%c is now orphaned", i);
			}
			else
			{
				orphaned_umodes &= ~prev_user_modes[i];
				sendto_realops_snomask(SNO_DEBUG, L_ALL,
						       "Orphaned umode +%c is picked up by module",
						       i);
			}
...
void
change_nick_user_host(struct Client *target_p, const char *nick, const char *user, const char *host,
		      int newts, const char *format, ...)
{
	rb_dlink_node *ptr;
	...
	char mode[6], modeval[(NICKLEN * 5) + 6], reason[REASONLEN], *mptr;
	va_list ap;
	...
	if(do_qjm)
	{
		...
		RB_DLINK_FOREACH(ptr, target_p->user->channel.head)
		{
			...
			*mptr = '\0';
			...
			sendto_channel_local_butone(target_p, ALL_MEMBERS, chptr,
						    ":%s!%s@%s JOIN :%s", nick, user, host,
						    chptr->chname);
			if(*mode)
				sendto_channel_local_butone(target_p, ALL_MEMBERS, chptr,
							    ":%s MODE %s +%s %s",
							    target_p->servptr->name, chptr->chname,
							    mode, modeval);
		...
		if(MyClient(target_p) && changed_case)
			sendto_one(target_p, ":%s!%s@%s NICK %s", target_p->name,
				   target_p->username, target_p->host, nick);
	}
	else if(changed_case)
	{
		sendto_common_channels_local(target_p, NOCAPS, ":%s!%s@%s NICK :%s", target_p->name,
					     target_p->username, target_p->host, nick);
	}
