...
get_or_create_channel(struct Client *client_p, const char *chname, bool * isnew)
{
	...
	/* Init the list modes */
	channel_create_list(chptr);

	hookdata.client = client_p;
	hookdata.chptr = chptr;
	hookdata.cmd = 0;
	hookdata.data = NULL;
	call_hook(h_channel_created, &hookdata);
...
output_hash(struct Client *source_p, const char *name, int length, int *counts, int deepest)
{
	...
	snprintf(buf, sizeof buf, "%.3f%%", (float) ((counts[0] * 100) / (float) length));
	sendto_one_numeric(source_p, RPL_STATSDEBUG, "B :Size: %d Empty: %d (%s)", length,
			   counts[0], buf);
	...
	if(counts[0] != length)
	{
		snprintf(buf, sizeof buf, "%.3f/%.3f", (float) (total / (length - counts[0])),
			 (float) (total / length));
	...
	for(i = 0; i < 11; i++)
	{
		sendto_one_numeric(source_p, RPL_STATSDEBUG, "B :Nodes with %d entries: %d", i,
				   counts[i]);
	}
}
