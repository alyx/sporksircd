...
mapi_hlist_av1 message_hlist[] = {
	{ "doing_channel_message", &doing_channel_message },
	{ NULL, NULL}
};

DECLARE_MODULE_AV1(message, modinit, moddeinit, message_clist, message_hlist, NULL, "SporksIRCD development team");
...
** -db Nov 13, 2000
**
** Cleaned up again. This time added a hook to do various filtering stuff in modules.
** --Elizabeth 24 Dec. 2010
*/
	...
	for(nick = rb_strtok_r(target_list, ",", &p); nick; nick = rb_strtok_r(NULL, ",", &p))
	{
		...
		if(type != 0)
		{
			...
			if((chptr = find_channel(nick)) != NULL)
			{
				...
				if(!IsServer(source_p) && !IsService(source_p) && !is_op_voiced(msptr))
				{
	    ...
	    const char *text)
{
	...
	result = can_send(chptr, source_p, NULL, text2, p_or_n == NOTICE ? COMMAND_NOTICE : COMMAND_PRIVMSG);
	if(result != CAN_SEND_NO && result != CAN_SEND_NO_NONOTIFY)
	{
		struct membership *msptr;
		text = text2;
		...
		{
			/* XXX using this seems like a bit of a hack to me and in fact rather stupid,
			 * but we can't specify if a hook goes last or whatever, and we need the above
			 * filtering, so we do this.
			 * *sigh*  --Elizabeth */
			hook_data_channel data;

			data.client = source_p;
			data.chptr = chptr;
			data.cmd = (p_or_n == NOTICE ? COMMAND_NOTICE : COMMAND_PRIVMSG);
			data.data = text;

			sendto_channel_flags(client_p, ALL_MEMBERS, source_p, chptr,
					     "%s %s :%s", command, chptr->chname, text);

			/* Call the hook. -sigh- --Elizabeth */
			call_hook(doing_channel_message, &data);
		}
	}
	...
	{
		if(p_or_n != NOTICE && (result != CAN_SEND_NO_NONOTIFY))
		  ...
		  struct Channel *chptr, const char *text)
{
	...
	/* XXX Mostly for the filtering this provides */
	can_send(chptr, source_p, NULL, text2, p_or_n == NOTICE ? COMMAND_NOTICE : COMMAND_PRIVMSG);
		  ...
		  struct Client *source_p, struct Channel *chptr, int flags, const char *text)
{
	...
	if(flags & CHFL_VOICE)
	{
		type = CHFL_VOICE | CHFL_HALFOP | CHFL_CHANOP | CHFL_ADMIN | CHFL_FOUNDER;
		c = '+';
	}
	else if(flags & CHFL_HALFOP)
	{
		type = CHFL_HALFOP | CHFL_CHANOP | CHFL_ADMIN | CHFL_FOUNDER;
		c = '%';
	}
	else if(flags & CHFL_CHANOP)
	{
		type = CHFL_CHANOP | CHFL_ADMIN | CHFL_FOUNDER;
		c = '@';
	}
	else if(flags & CHFL_ADMIN)
	{
		type = CHFL_ADMIN | CHFL_FOUNDER;
		c = '!';
	}
	else
	{
...
flood_attack_client(int p_or_n, struct Client *source_p, struct Client *target_p)
{
	 ...
	 * -- jilles
	 */
	if(GlobalSetOptions.floodcount && IsClient(source_p) &&
	   source_p != target_p && !IsService(target_p) &&
