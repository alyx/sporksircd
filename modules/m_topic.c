...
DECLARE_MODULE_AV1(topic, NULL, NULL, topic_clist, NULL, NULL, "SporksIRCD development team");
...
m_topic(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(parc > 2)
	{
		...
		if(MyClient(source_p) && !is_op_voiced(msptr) &&
				!IsOper(source_p) &&
		...
		cansend = can_send(chptr, source_p, msptr, topic, COMMAND_TOPIC);

		if(((chptr->mode.mode & MODE_TOPICLIMIT) == 0 ||
		   get_channel_access(source_p, msptr) >= CHFL_HALFOP) &&
		   (!MyClient(source_p) ||
		    ((cansend != CAN_SEND_NO) && cansend != (CAN_SEND_NO_NONOTIFY))))
		{
		...
		{
			if(cansend != CAN_SEND_NO_NONOTIFY)
