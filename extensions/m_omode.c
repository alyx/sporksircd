...
DECLARE_MODULE_AV1(omode, NULL, NULL, omode_clist, NULL, NULL, "SporksIRCD development team");
...
mo_omode(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	/* We let non-founders set modes in case they have to found themselves 
	 * This is generally for emergency use only... */
	if (is_founder(msptr))
	{
		sendto_one_notice(source_p,
				  ":Use a normal MODE you fucktard (you have founder status and don't need OMODE)");
		return 0;
	}
	...
	set_channel_mode(client_p, source_p->servptr, chptr, msptr, 
			 parc - 2, parv + 2);
#else
	if (parc == 4 && !strcmp(parv[2], "+u") && !irccmp(parv[3], source_p->name))
	{
	        /* Ownering themselves */
	        if (!wasonchannel)
	        {
