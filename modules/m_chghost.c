...
DECLARE_MODULE_AV1(chghost, NULL, NULL, chghost_clist, NULL, NULL, "SporksIRCD development team");
...
do_chghost(struct Client *source_p, struct Client *target_p,
		const char *newhost, bool is_encap)
{
	...
	int parc, const char *parv[])
{
	...
	if (do_chghost(source_p, target_p, parv[2], NO))
	{
	...
	int parc, const char *parv[])
{
	...
	do_chghost(source_p, target_p, parv[2], YES);
	...
	int parc, const char *parv[])
{
	...
	do_chghost(source_p, target_p, parv[2], NO);
