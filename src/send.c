...
send_queued(struct Client *to)
{
	...
	{
		while((retlen = rb_linebuf_flush(F, &to->localClient->buf_sendq)) > 0)
		{
 ...
 */
void
sendto_one_prefix(struct Client *target_p, struct Client *source_p, const char *command,
		  const char *pattern, ...)
{
	...
	rb_linebuf_newbuf(&linebuf);
	va_start(args, pattern);
	rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s %s %s ", get_id(source_p, target_p),
...
sendto_one_notice(struct Client *target_p, const char *pattern, ...)
{
	...
	rb_linebuf_newbuf(&linebuf);
	va_start(args, pattern);
	rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s NOTICE %s ", get_id(&me, target_p),
...
sendto_one_numeric(struct Client *target_p, int numeric, const char *pattern, ...)
{
	...
	rb_linebuf_newbuf(&linebuf);
	va_start(args, pattern);
	rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s %03d %s ", get_id(&me, target_p), numeric,
			  *(to = get_id(target_p, target_p)) != '\0' ? to : "*");
	va_end(args);
 ...
 */
void
sendto_server(struct Client *one, struct Channel *chptr, unsigned long caps, unsigned long nocaps,
	      const char *format, ...)
{
	va_list args;
 ...
 */
void
sendto_channel_flags(struct Client *one, int type, struct Client *source_p, struct Channel *chptr,
		     const char *pattern, ...)
{
	...
	if(IsServer(source_p))
		...
		rb_linebuf_putmsg(&rb_linebuf_local, NULL, NULL, ":%s!%s@%s %s", source_p->name,
				  source_p->username, source_p->host, buf);
 ...
 */
void
sendto_channel_opmod(struct Client *one, struct Client *source_p, struct Channel *chptr,
		     const char *command, const char *text)
{
	...
	if(IsServer(source_p))
		...
		rb_linebuf_putmsg(&rb_linebuf_local, NULL, NULL, ":%s!%s@%s %s %s :%s",
				  source_p->name, source_p->username, source_p->host, command,
				  chptr->chname, text);

	if(chptr->mode.mode & MODE_MODERATED)
		rb_linebuf_putmsg(&rb_linebuf_old, NULL, NULL, ":%s %s %s :%s", use_id(source_p),
				  command, chptr->chname, text);
	else
		rb_linebuf_putmsg(&rb_linebuf_old, NULL, NULL, ":%s NOTICE @%s :<%s:%s> %s",
				  use_id(source_p->servptr), chptr->chname, source_p->name,
				  chptr->chname, text);
	rb_linebuf_putmsg(&rb_linebuf_new, NULL, NULL, ":%s %s =%s :%s", use_id(source_p), command,
			  chptr->chname, text);
 ...
 */
void
sendto_channel_local_with_capability(int type, int caps, int negcaps, struct Channel *chptr,
				     const char *pattern, ...)
{
	va_list args;
	buf_head_t linebuf;
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, chptr->locmembers.head)
	{
		...
		if(IsIOError(target_p) || !IsCapable(target_p, caps)
 ...
 */
void
sendto_channel_local_butone(struct Client *one, int type, struct Channel *chptr,
			    const char *pattern, ...)
{
	va_list args;
	buf_head_t linebuf;
 ...
 */
void
sendto_match_butone(struct Client *one, struct Client *source_p, const char *mask, int what,
		    const char *pattern, ...)
{
	static char buf[BUFSIZE];
	va_list args;
 ...
 */
void
sendto_match_servs(struct Client *source_p, const char *mask, int cap, int nocap,
		   const char *pattern, ...)
{
	static char buf[BUFSIZE];
	va_list args;
 ...
 */
void
sendto_anywhere(struct Client *target_p, struct Client *source_p, const char *command,
		const char *pattern, ...)
{
	va_list args;
	buf_head_t linebuf;
	...
	{
		if(IsServer(source_p))
			rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s %s %s ", source_p->name,
					  command, target_p->name);
		else
			rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s!%s@%s %s %s ",
...
sendto_realops_snomask(int flags, int level, const char *pattern, ...)
{
	...
	if(level & L_NETWIDE && ConfigFileEntry.global_snotices)
	{
		...
		rb_vsnprintf(buf, sizeof(buf), pattern, args);
		va_end(args);
		rb_linebuf_putmsg(&linebuf, pattern, NULL, ":%s NOTICE * :*** Notice -- %s",
				  me.name, buf);
		snobuf = construct_snobuf(flags);
		if(snobuf[1] != '\0')
			sendto_server(NULL, NULL, CAP_ENCAP | CAP_TS6, NOCAPS,
				      ":%s ENCAP * SNOTE %c :%s", me.id, snobuf[1], buf);
	}
	...
	{
		va_start(args, pattern);
		rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s NOTICE * :*** Notice -- ",
				  me.name);
		va_end(args);
	}
	level &= ~L_NETWIDE;
 ...
 */
void
sendto_realops_snomask_from(int flags, int level, struct Client *source_p, const char *pattern, ...)
{
	struct Client *client_p;
	rb_dlink_node *ptr;
	rb_dlink_node *next_ptr;
...
sendto_wallops_flags(int flags, struct Client *source_p, const char *pattern, ...)
{
	...
	if(IsPerson(source_p))
		rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s!%s@%s WALLOPS :", source_p->name,
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, IsPerson(source_p)
			      && flags == UMODE_WALLOP ? lclient_list.head : local_oper_list.head)
	{
		client_p = ptr->data;
...
kill_client(struct Client *target_p, struct Client *diedie, const char *pattern, ...)
{
	...
	va_start(args, pattern);
	rb_linebuf_putmsg(&linebuf, pattern, &args, ":%s KILL %s :", get_id(&me, target_p),
			  get_id(diedie, target_p));
	va_end(args);
...
kill_client_serv_butone(struct Client *one, struct Client *target_p, const char *pattern, ...)
{
	...
	rb_linebuf_putmsg(&rb_linebuf_id, NULL, NULL, ":%s KILL %s :%s", use_id(&me),
			  use_id(target_p), buf);
