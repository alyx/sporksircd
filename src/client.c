...
struct rb_dictionary *nd_dict = NULL;
...
init_client(void)
{
	...
	nd_dict = rb_dictionary_create(irccmp);
}
			    ...
			    (client_p->flags & FLAGS_PINGSENT)))
			{
				if(IsServer(client_p))
				{
					sendto_realops_snomask(SNO_GENERAL, L_ALL,
							       ...
							       client_p->name);
					ilog(L_SERVER, "No response from %s, closing link",
		...
		if((rb_current_time() - client_p->localClient->firsttime) > timeout)
		{
			if(IsAnyServer(client_p))
			{
				sendto_realops_snomask(SNO_GENERAL,
						       is_remote_connect(client_p) ? L_NETWIDE :
						       L_ALL, "No response from %s, closing link",
						       client_p->name);
				ilog(L_SERVER, "No response from %s, closing link",
				     log_client_name(client_p, HIDE_IP));
			}
...
notify_banned_client(struct Client *client_p, struct ConfItem *aconf, int ban)
{
	...
	if(ban == D_LINED && !IsPerson(client_p))
		...
		sendto_one(client_p, form_str(ERR_YOUREBANNEDCREEP), me.name, client_p->name,
			   reason);

	exit_client(client_p, client_p, &me,
		    EmptyString(ConfigFileEntry.kline_reason) ? exit_reason : ConfigFileEntry.
		    kline_reason);
}
...
check_klines(void)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, lclient_list.head)
	{
		...
		if((aconf = find_kline(client_p)) != NULL)
		{
			...
			sendto_realops_snomask(SNO_GENERAL, L_ALL, "KLINE active for %s",
					       get_client_name(client_p, HIDE_IP));
...
check_dlines(void)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, lclient_list.head)
	{
		...
		if((aconf =
		    find_dline((struct sockaddr *) &client_p->localClient->ip,
			       client_p->localClient->ip.ss_family)) != NULL)
		{
			if(aconf->status & CONF_EXEMPTDLINE)
				continue;

			sendto_realops_snomask(SNO_GENERAL, L_ALL, "DLINE active for %s",
					       get_client_name(client_p, HIDE_IP));
...
update_client_exit_stats(struct Client *client_p)
{
	if(IsServer(client_p))
	{
		sendto_realops_snomask(SNO_EXTERNAL, L_ALL, "Server %s split from %s",
...
get_client_name(struct Client *client, int showip)
{
	...
	if(MyConnect(client))
	{
		...
		{
		case SHOW_IP:
			rb_snprintf(nbuf, sizeof(nbuf), "%s[%s@%s]", client->name, client->username,
				    client->sockhost);
			break;
		case MASK_IP:
			rb_snprintf(nbuf, sizeof(nbuf), "%s[%s@255.255.255.255]", client->name,
				    client->username);
			break;
		default:
			rb_snprintf(nbuf, sizeof(nbuf), "%s[%s@%s]", client->name, client->username,
...
log_client_name(struct Client *target_p, int showip)
{
	...
	if(MyConnect(target_p))
	{
		...
		case MASK_IP:
			rb_snprintf(nbuf, sizeof(nbuf), "%s[%s@255.255.255.255]", target_p->name,
				    target_p->username);
			...
			RB_DLINK_FOREACH(aptr, abort_list.head)
			{
				...
				if(abt->client == target_p)
				{
					...
					sendto_realops_snomask(SNO_GENERAL, L_ALL,
							       ...
							       target_p->flags, target_p->flags2,
							       target_p->handler);
					sendto_realops_snomask(SNO_GENERAL, L_ALL,
							       "Please report this to the SporksIRCd developers!");
					found = YES;
				}
			}
...
*/
static void
recurse_send_quits(struct Client *client_p, struct Client *source_p, struct Client *to,
		   const char *comment1, const char *comment)
{
	struct Client *target_p;
...
*/
static void
remove_dependents(struct Client *client_p, struct Client *source_p, struct Client *from,
...
exit_aborted_clients(void *unused)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next, abort_list.head)
	{
...
#ifdef DEBUG_EXITED_CLIENTS
		{
			if(rb_dlinkFind(abt->client, &dead_list))
			{
				...
				sendto_realops_snomask(SNO_GENERAL, L_ALL,
						       ...
						       abt->client->flags, abt->client->flags2,
						       abt->client->handler);
				sendto_realops_snomask(SNO_GENERAL, L_ALL,
						       "Please report this to the SporksIRCD developers!");
				continue;
		...
		if(IsAnyServer(abt->client))
			sendto_realops_snomask(SNO_GENERAL, L_ALL, "Closing link to %s: %s",
		    ...
		    const char *comment)
{
	...
	sendto_common_channels_local(source_p, NOCAPS, ":%s!%s@%s QUIT :%s", source_p->name,
		   ...
		   const char *comment)
{
	...
	{
		sendto_server(client_p, NULL, CAP_TS6, NOCAPS, ":%s QUIT :%s", use_id(source_p),
			      comment);
	}
		   ...
		   const char *comment)
{
	...
	if(source_p->serv != NULL)
		remove_dependents(client_p, source_p, from, IsPerson(from) ? newcomment : comment,
				  comment1);
	...
	if(target_p != NULL && IsServer(target_p) && target_p != client_p && !IsMe(target_p) &&
	   (source_p->flags & FLAGS_KILLED) == 0)
	{
		sendto_one(target_p, ":%s SQUIT %s :%s", get_id(from, target_p),
			   get_id(source_p, target_p), comment);
	}
		  ...
		  const char *comment)
{
	...
	{
		sendto_one(source_p, "ERROR :Closing Link: 127.0.0.1 %s (%s)", source_p->name,
			   comment);
	}
	...
	sendto_realops_snomask(SNO_GENERAL, L_ALL, "%s was connected"
			       " for %ld seconds.  %d/%d sendK/recvK.", source_p->name,
			       (long) rb_current_time() - source_p->localClient->firsttime, sendk,
			       recvk);

	ilog(L_SERVER, "%s was connected for %ld seconds.  %d/%d sendK/recvK.", source_p->name,
	     (long) rb_current_time() - source_p->localClient->firsttime, sendk, recvk);
		  ...
		  const char *comment)
{
	...
	sendto_realops_snomask(SNO_CCONNEXT, L_ALL,
			       "CLIEXIT %s %s %s %s 0 %s", source_p->name, source_p->username,
			       source_p->host, show_ip(NULL, source_p) ? source_p->sockhost : "255.255.255.255",
			       comment);
	...
	ilog(L_USER, "%s (%3lu:%02lu:%02lu): %s!%s@%s %s %d/%d",
	     ...
	     (on_for % 3600) / 60, on_for % 60, source_p->name, source_p->username, source_p->host,
	     source_p->sockhost, source_p->localClient->sendK, source_p->localClient->receiveK);
	...
	if(--user->refcnt <= 0)
	{
		...
		if(user->refcnt < 0 || user->invited.head || user->channel.head)
		{
			sendto_realops_snomask(SNO_GENERAL, L_ALL,
					       ...
					       rb_dlink_list_length(&user->channel), user->refcnt);
...
error_exit_client(struct Client *client_p, int error)
{
	...
	if(IsServer(client_p) || IsHandshake(client_p))
	{
		if(error == 0)
		{
			sendto_realops_snomask(SNO_GENERAL, is_remote_connect(client_p)
					       && !IsServer(client_p) ? L_NETWIDE : L_ALL,
		...
		else
		{
			sendto_realops_snomask(SNO_GENERAL, is_remote_connect(client_p)
					       && !IsServer(client_p) ? L_NETWIDE : L_ALL,
					       "Lost connection to %s: %s", client_p->name,
...
user_metadata_add(struct Client *target, const char *name, const char *value, int propagate)
{
	...
	rb_dictionary_add(target->user->metadata, md->name, md);
...
user_metadata_delete(struct Client *target, const char *name, int propagate)
{
	...
	rb_dictionary_delete(target->user->metadata, md->name);
...
user_metadata_find(struct Client *target, const char *name)
{
	...
	return rb_dictionary_retrieve(target->user->metadata, name);
}
...
user_metadata_clear(struct Client *target)
{
	...
	RB_DICTIONARY_FOREACH(md, &iter, target->user->metadata)
	{
