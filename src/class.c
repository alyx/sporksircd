...
report_classes(struct Client *source_p)
{
	...
	RB_DLINK_FOREACH(ptr, class_list.head)
	{
		...
		sendto_one_numeric(source_p, RPL_STATSYLINE,
				   ...
				   ConFreq(cltmp), MaxUsers(cltmp), MaxSendq(cltmp),
				   MaxLocal(cltmp), MaxIdent(cltmp), MaxGlobal(cltmp),
				   MaxIdent(cltmp), CurrUsers(cltmp));
	}
	...
	sendto_one_numeric(source_p, RPL_STATSYLINE, form_str(RPL_STATSYLINE),
			   ...
			   MaxSendq(default_class), MaxLocal(default_class),
			   MaxIdent(default_class), MaxGlobal(default_class),
			   MaxIdent(default_class), CurrUsers(default_class));
}
