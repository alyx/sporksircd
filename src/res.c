		...
		case AF_INET:
			if(srv->ss_family == inp->ss_family)
				if(v4->sin_port == v4in->sin_port)
					if((v4->sin_addr.s_addr == INADDR_ANY) || (v4->sin_addr.s_addr == v4in->sin_addr.s_addr))
					{
						ns_timeout_count[ns] = 0;
						return 1;
					}
...
start_resolver(void)
{
	...
	{
		if((res_fd =
		    rb_socket(irc_nsaddr_list[0].ss_family, SOCK_DGRAM, 0,
...
send_res_msg(const char *msg, int len, int rcount)
{
	...
	for(i = 0; i < irc_nscount; i++)
	{
		...
		if(ns_timeout_count[ns] && retrycnt % retryfreq(ns_timeout_count[ns]))
			continue;
		if(sendto(rb_get_fd(res_fd), msg, len, 0, (struct sockaddr *) &(irc_nsaddr_list[ns]), GET_SS_LEN(&irc_nsaddr_list[ns])) == len)
			return ns;
	}
	...
	for(i = 0; i < irc_nscount; i++)
	{
		...
		if(!ns_timeout_count[ns])
			continue;
		if(sendto
		   (rb_get_fd(res_fd), msg, len, 0, (struct sockaddr *) &(irc_nsaddr_list[ns]),
		...
		struct reslist *request)
{
	...
	else if(addr->ss_family == AF_INET6)
	{
		...
		(void) sprintf(request->queryname,
...
proc_answer(struct reslist *request, HEADER * header, char *buf, char *eob)
{
	...
	while(header->ancount > 0 && (char *) current < eob)
	{
		...
		default:
			 ...
			 */
			ilog(L_MAIN, "res.c bogus type %d", type);
			break;
		}
	}
...
report_dns_servers(struct Client *source_p)
{
	...
	for(i = 0; i < irc_nscount; i++)
	{
		...
		sendto_one_numeric(source_p, RPL_STATSDEBUG, "A %s %d", ipaddr,
				   ns_timeout_count[i]);
	}
}
